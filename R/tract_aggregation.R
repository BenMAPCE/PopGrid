tract_aggregation <- function(
  ...
  # add COL (tract FIPS) and ROW (state + county FIPS) columns, although unintuitive this is how 2010 CT was setup
  out_data <- raw_data %>% mutate(Column = substr(GEOID, 6, 11), Row = substr(GEOID, 1, 5))
